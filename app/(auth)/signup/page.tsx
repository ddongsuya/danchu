...
function Signup() {
  ...
  const ok = f.company && f.name && f.email && f.password.length >= 8 && agree;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await postJson("/api/auth/signup", { ...f, next });
      setDone(true);
    } catch (err) {
    ...
    return (
      <div className="auth__card">
        <Done title="확인 메일을 보냈습니다">
          <p>
            <b>{f.email}</b>로 보낸 메일의 버튼을 누르면 가입이 완료되고 바로 로그인됩니다.
          </p>
  ...
  return (
    <div className="auth__card auth__card--wide">
      <div>
        <h1 className="auth__title">의뢰자 가입</h1>
        <p className="auth__sub">계정을 만들면 요청 진행 상황, 도착한 견적, 비교표를 한곳에서 볼 수 있습니다. 무료입니다.</p>
      </div>
      ...
      <form className="auth__form" onSubmit={submit}>
        ...
        <Field id="password" label="비밀번호" required help="8자 이상. 이메일 링크로만 로그인하셔도 됩니다.">
