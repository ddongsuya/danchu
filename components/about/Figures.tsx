  ...
  return (
    <figure className="fig rv" data-rv>
      <svg viewBox="0 0 760 300" role="img" aria-labelledby="fig1t" className="fig__svg">
        ...
        <text {...A(1.2)} x="176" y="272" fontSize="11.5" fill={F.muted} textAnchor="middle">같은 설명 4번 · 양식이 달라 비교 불가</text>
        ...
        <path {...L(1.85)} d="M508 150 L 536 150" stroke={F.brand} strokeWidth="1.6" />
        <g {...A(2.0)}>
        ...
        {croY.map((y, i) => (
          <g key={`b${y}`}>
            <path {...L(2.15 + i * 0.1)} d={`M578 150 C 600 150, 606 ${y + 20}, 626 ${y + 20}`} fill="none" stroke={F.brand} strokeWidth="1.2" opacity="0.55" />
            <rect {...A(2.35 + i * 0.1)} x="626" y={y + 6} width="28" height="28" rx="14" fill={F.white} stroke="var(--brand-line)" />
          </g>
        ))}
        <g {...A(2.8)}>
        ...
        <text {...A(3.4)} x="584" y="272" fontSize="11.5" fill={F.brand} textAnchor="middle">한 번 입력 · 같은 형식으로 비교</text>
      </svg>
  ...
  return (
    <figure className="fig rv" data-rv>
      <svg viewBox="0 0 760 300" role="img" aria-labelledby="fig2t" className="fig__svg">
        <title id="fig2t">요청서에서 고른 시험 항목이 그대로 모든 기관 회신 표의 행이 되고, 기관은 같은 칸을 채웁니다</title>
        ...
        <text {...A(0.8)} x="20" y="258" fontSize="11" fill={F.muted}>대분류만 골라도 접수됩니다</text>

        <g {...A(0.9)}>
          <text x="370" y="14" fontSize="13" fontWeight="700" fill={F.muted}>모든 기관의 회신 표</text>
        ...
        {/* 연결 */}
        {[0, 1, 2].map((i) => (
          <path key={i} {...L(1.1 + i * 0.22)} d={`M300 ${95 + i * 58} C 330 ${95 + i * 58}, 340 ${100 + i * 52}, 370 ${100 + i * 52}`} fill="none" stroke={F.brand} strokeWidth="1.3" opacity="0.5" />
        ))}
        <g {...A(1.1)}>
          <text x="335" y="34" fontSize="11" fontWeight="600" fill={F.brand} textAnchor="middle">같은</text>
          <text x="335" y="48" fontSize="11" fontWeight="600" fill={F.brand} textAnchor="middle">행</text>
        </g>
        {items.map(([t], i) => (
        ...
        <text {...A(2.1)} x="390" y="252" fontSize="11" fill={F.muted}>어느 기관이든 같은 칸을 채우고 정식 견적서 PDF를 첨부합니다</text>
      </svg>
  ...
  return (
    <figure className="fig rv" data-rv>
      <svg viewBox="0 0 760 250" role="img" aria-labelledby="fig3t" className="fig__svg">
        ...
        <path {...L(0.35)} d="M200 90 L 268 90" stroke={F.brand} strokeWidth="1.6" />
        <g {...A(0.55)}>
        ...
        {/* 체결 기관 */}
        <path {...L(0.85)} d="M310 84 C 350 84, 360 56, 400 56" fill="none" stroke={F.brand} strokeWidth="1.4" />
        <g {...A(1.05)}>
        ...
        <path {...L(1.2)} d="M310 92 C 350 92, 360 116, 400 116" fill="none" stroke={F.brand} strokeWidth="1.4" />
        <g {...A(1.4)}>
        ...
        <text {...A(2.1)} x="0" y="232" fontSize="11.5" fill={F.muted}>선택한 CRO에만 연락처가 공개되고, 어느 기관도 다른 기관의 견적을 볼 수 없습니다.</text>
      </svg>
